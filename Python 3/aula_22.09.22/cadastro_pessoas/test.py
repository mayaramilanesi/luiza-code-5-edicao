from cadastro_pessoas import user_add
...
def test_valida_usuario_existente_error(monkeypatch, capsys):
      ...
      monkeypatch.setattr("src.cadastro_pessoas.users", [{"name": "Michelly Alves", "age": 27}])
...
def test_vallida_usuario_sucess(monkeypatch, capsys):
      inputs = iter(["Michelly Alves", 27])
      monkeypatch.setattr("builtins.input", lambda _: next(inputs))
